    ...
    for vid_url in vids_urls:
        ...
        if download:
            ...
            try:
                ...
                row_vid.downloaded = True ### mark as downloaded on Notion if success
                # row_vid.ignore = False ### just in case
                ...
                print(e)
                # row_vid.ignore = True ### mark as to be ignored on Notion if failure (not mandatory since every non marked as downloaded is trying again forever)
...
def attemptStreamDownload(full_url, row_vid, channel_name, ignore_higher_bitrate=True, audio_only=False):
    ...
    # if row_vid is not None: row_vid.downloaded = True ### duplicate : is now done in wrapper method (downloadVideosFromLinks)
    if not audio_only: print("video at [{}] has finished downloading at attempt n°{} — [ {} ]".format(full_url, attempt, bitrate))
    elif audio_only  : print("audio at [{}] has finished downloading.".format(full_url))